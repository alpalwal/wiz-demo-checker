...
resource "aws_sns_topic_subscription" "demo_issue_topic_subscription2" {
  topic_arn = resource.aws_sns_topic.demo_issue_topic.arn
  protocol  = "email"
  endpoint  = "jonathan.dong@wiz.io"
}

resource "aws_sns_topic_subscription" "demo_issue_topic_subscription3" {
  topic_arn = resource.aws_sns_topic.demo_issue_topic.arn
  protocol  = "email"
  endpoint  = "avi.konstantini@wiz.io"
}

resource "aws_sns_topic_subscription" "demo_issue_topic_subscription4" {
  topic_arn = resource.aws_sns_topic.demo_issue_topic.arn
  protocol  = "email"
  endpoint  = "bryan.mcclellan@wiz.io"
}

resource "aws_sns_topic_subscription" "demo_issue_topic_subscription5" {
  topic_arn = resource.aws_sns_topic.demo_issue_topic.arn
  protocol  = "email"
